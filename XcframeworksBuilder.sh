...
XCFrameworks_Output_Folder="Xcframeworks_Output"
...
# Workspace path
WorkSpace_Path="JMMediaSampleApp/JMMediaSampleApp.xcworkspace"
...
xcodebuild archive ONLY_ACTIVE_ARCH=NO -workspace ${WorkSpace_Path} -scheme ${BUILD_SCHEME} -destination="generic/platform=iOS Simulator" -archivePath "${SIMULATOR_ARCHIVE_PATH}" -sdk iphonesimulator SKIP_INSTALL=NO BUILD_LIBRARIES_FOR_DISTRIBUTION=YES

# Create Device Destination Archive
xcodebuild archive -workspace ${WorkSpace_Path} -scheme ${BUILD_SCHEME} -destination="generic/platform=iOS" -archivePath "${IOS_DEVICE_ARCHIVE_PATH}" -sdk iphoneos SKIP_INSTALL=NO BUILD_LIBRARIES_FOR_DISTRIBUTION=YES
